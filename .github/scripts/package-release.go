// Package-release packs a CPA plugin dynamic library into the Plugins Store zip layout.
//
//	go run ./.github/scripts/package-release.go \
//	  -library dist/freebuff.so \
//	  -archive freebuff_0.1.0_linux_amd64.zip \
//	  -checksum freebuff_0.1.0_linux_amd64.zip.sha256
//
// Zip root must contain exactly one library named <id>.{so,dylib,dll}.
package main
...
func packageLibrary(libraryPath, archivePath string) ([]byte, error) {
	...
	// Store library at zip root with its basename (freebuff.so / .dll / .dylib).
